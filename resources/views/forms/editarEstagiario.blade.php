...
@endphp
    <div class="mt-20 space-y-10">
        ...
        <x-Title-app title="Estagiários > Editar" icon="bi bi-people-fill" action="/estagiarios" type="secondary" text-action="Voltar" />
                ...
                <form action="/estagiarios/atualizar/" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div>
                        ...
                        <div class="flex items-center border border-slate-300 rounded-lg">
                            ...
                            <input type="text" id="nome" name="nome" value="{{ $estagiario->nome }}" maxlength="50" required pattern="^[A-Za-zÀ-ÿ\s]{3,50}$" class="{{ $class_input }}" placeholder="Exemplo: João Silva">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center border border-slate-300 rounded-lg">
                            ...
                            <input type="email" id="email" name="email" value="{{ $estagiario->email }}" pattern="^[a-zA-Z0-9._%+-]+@[a-zA-Z0-9.-]+\.[a-z]{2,}$" class="{{ $class_input }}" placeholder="Exemplo: email@dominio.com">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <div class="flex items-center border border-slate-300 rounded-lg">
                                ...
                                <input type="text" id="tel" name="tel" value="{{ $estagiario->tel }}" maxlength="9" required pattern="^9\d{8}$" class="{{ $class_input }}" placeholder="9 dígitos começando por 9">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="flex items-center border border-slate-300 rounded-lg">
                                ...
                                <input type="text" id="bi" name="bi" value="{{ $estagiario->bi }}" maxlength="14" required pattern="^\d{9}[A-Z]{2}\d{3}$" class="{{ $class_input }}" placeholder="006984317LA098">
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Data de Nascimento -->
                    <div>
                        <label for="dt_nascimento" class="block text-sm font-medium text-slate-600 mb-1">Data de Nascimento</label>
                        <div class="flex items-center border border-slate-300 rounded-lg">
                            <i class="bi-calendar-fill text-slate-400 p-2"></i>
                            <input type="date" id="dt_nascimento" name="dt_nascimento" value="{{ $estagiario->dt_nascimento }}" required class="{{ $class_input }}" min="1980-01-01" max="2015-01-01">
                        </div>
                    </div>
                        ...
                        <div class="flex items-center border border-slate-300 rounded-lg">
                            ...
                            <select id="plano" name="plano" required class="{{ $class_input }}">
                                @foreach ($planos as $plano)
                                    <option value="{{ $plano->id }}" @if($estagiario->plano_estagio_id == $plano->id) selected @endif>{{ $plano->nome }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <!-- Instituto -->
                    <div>
                        <label for="instituto" class="block text-sm font-medium text-slate-600 mb-1">Instituto de Origem (Opcional)</label>
                        <div class="flex items-center border border-slate-300 rounded-lg">
                            <i class="bi-buildings-fill text-slate-400 p-2"></i>
                            <select id="instituto" name="instituto" class="{{ $class_input }}">
                                @foreach ($institutos as $instituto)
                                    <option value="{{ $instituto->id }}" @if($estagiario->instituto_id == $instituto->id) selected @endif>{{ $instituto->nome }}</option>
                                @endforeach
                    ...
                    <div>
                        ...
                        <x-bladewind::filepicker name="foto" accepted_file_types="image/*" max_file_size="10mb" placeholder="Foto passe"  selected_value="{{asset('uploads/'.$estagiario->foto)}}" required/>
                    </div>

                    <!-- Documentos -->
                    <div>
                        <label for="documentos" class="block text-sm font-medium text-slate-600 mb-1">Documentos (PDF, DOC, DOCX)</label>
                        <x-bladewind::filepicker name="documentos" accepted_file_types="application/pdf, .doc, .docx" max_file_size="10mb" placeholder="Documentos" selected_value="{{asset('uploads/'.$estagiario->documentos)}}" />
                    </div>
