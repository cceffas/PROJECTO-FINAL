                ...
                <div class="flex flex-col items-center justify-center">
                    <form action="/turmas/deletar/{{ $turma->id }}" method="get">
                        ...
                        <div class="flex gap-4 mt-4">
                            <x-bladewind::button type="secondary" onclick="hideModal('{{ $turma->id }}')">não</x-bladewind::button>
                ...
                <x-bladewind::card>
                    <div>
                        <form action="/turmas/atualizar" method="post">
                            ...
                            {{-- end --}}
                            <x-bladewind::input type="text" label='Nome' name='nome' required value="{{ $turma->nome }}" />
                            <x-bladewind::input numeric=true label='numero maximo de alunos' name='qtd_aluno' min=8 value="{{ $turma->qtd_aluno}}" />
                            {{-- end --}}
                            <x-bladewind::card>
                                <h1 class="">Cursos</h1>
                                <div class="flex flex-wrap p-2">
                                    ...
                                    <x-bladewind::radio label="{{ $item->nome }}" value="{{$item->id}}" name='curso' checked=true />
                                    @else
                            ...
                            <div class="flex justify-end gap-4 mt-2">
                                <x-bladewind::button type='secondary' onclick="hideModal('{{ $turma->id }}edit')">cancelar</x-bladewind::button>
                ...
                <x-bladewind::card>
                    <div>
                        <form action="/turmas/criar" method="post">
                            ...
                            <div class="flex justify-end gap-4 mt-2">
                                <x-bladewind::button type='secondary' onclick="hideModal('turma')">cancelar</x-bladewind::button>
                                <x-bladewind::button can_submit=true>confirmar</x-bladewind::button>
                            </div>
                        </form>
                    </div>
